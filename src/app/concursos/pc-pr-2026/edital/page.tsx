...
import { dadosGeraisEdital, disciplinasAgente, DOCUMENTO_EDITAL, inscricoesEProva, regioesOficiais, URL_COMUNICADO_PRORROGACAO_PAGAMENTO, URL_FONTE_FGV, URL_REIMPRESSAO_BOLETO_FGV } from "@/data/edital";
  ...
  return (
    <InternalPage path="/concursos/pc-pr-2026/edital" title="Edital" description="Dados oficiais extraídos do edital cadastrado localmente. O PDF local não é publicado pelo portal." audited>
      ...
      <InfoGrid>
        ...
        <InfoCard><div className="flex items-start justify-between gap-3"><h2>Inscrição e pagamento</h2><DataBadge tipo="oficial" /></div><p>As inscrições permanecem encerradas em 12/08/2026 às 16h. A FGV prorrogou exclusivamente o prazo para pagamento da taxa de inscrição até {formatarPrazo(inscricoesEProva.boletoFim)}.</p><div className="internal-stat"><span>Comunicado</span><strong>{formatarData(inscricoesEProva.boletoProrrogadoEm)}</strong></div><a className="source-external-link ds-focusable" href={URL_REIMPRESSAO_BOLETO_FGV} target="_blank" rel="noopener noreferrer">Reimprimir boleto — FGV<ExternalLink aria-hidden="true" className="h-4 w-4" /><span className="sr-only">(abre em nova aba)</span></a><a className="source-external-link ds-focusable" href={URL_COMUNICADO_PRORROGACAO_PAGAMENTO} target="_blank" rel="noopener noreferrer">Consultar comunicado<ExternalLink aria-hidden="true" className="h-4 w-4" /><span className="sr-only">(abre em nova aba)</span></a></InfoCard>
